...
def missing_positive(nums):
    ...
    def _segregate(low, high):
        if low == high: # watch out this corner case!
            if nums[low] <= 0:
                return 1
            else:
                return 0
    ...
    len_n = len(nums)
    p_i = _segregate(0, len_n-1)
    if p_i >= len_n: # all negative
        return 1
        
    for i in range(p_i, len_n):
        index = nums[i]
        if index < 0:
            index *= -1
            index -= 1
